...
def mkLed():
    ...
    def comp(size):
        ...
        # fifo_c
        maxi_out.dma_write_async(1024, size >> 1)
        axi_out.read_fifo(fifo_c, size >> 1)
        ...
        myaxi.dma_write(ram_c, offset, 1024 * 2, size >> 1)

        # verification
        myaxi.dma_read(ram_c, 0, 1024, size >> 1)
        myaxi.dma_read(ram_c, offset, 1024 * 2, size >> 1)
        check(size >> 1, 0, offset)
